...
type TabKey = "overview" | "tasks" | "notes";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b border-gray-800">
        {(["overview", "tasks", "notes"] as TabKey[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
          ...
          >
            {t === "overview" ? "Overview & Results" : t === "tasks" ? `Tasks (${project.tasks.length})` : `Notes (${project.notes.length})`}
          </button>
        ))}
      </div>
