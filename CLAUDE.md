...
- **폐쇄망(air-gapped) 서버이다. git 명령어를 스크립트에 절대 넣지 않는다.** `git pull`, `git fetch`, `git clone`, `git push` 등 모든 git 네트워크 명령어 금지. 사용자가 로컬에서 pull → 파일을 서버로 복사하는 방식이다.
- **사용자에게 명령어를 직접 타이핑하라고 안내하지 않는다.** 항상 스크립트 파일로 만들어서 push한다.

## 능동 제안 규칙 (필수)

- 사용자가 반복적·수동적으로 하고 있는 작업(스크린샷 전달, 값 복사, 로그 붙여넣기 등)을 발견하면, 자동화·단축 방법을 즉시 먼저 제안할 것.
- 세션 시작 시 1회: 직전 작업 방식에서 비효율 후보가 보이면 언급할 것.
...
- `npm run test:run` (vitest): 존재하는 단위 테스트 11개 파일(약 72개)만 실행. 새 기능·React 렌더링·DB·Prometheus 연동은 **커버하지 않음**. (현재 수치는 `docs/stats.md` 참조 — SSOT)
