...
set_notes(slide, "인사 + 한 줄 목적: '흩어진 데이터센터 운영을 AI와 함께 통합 시스템으로 만든 사례를 공유하고, "
                 "이 방식을 어떻게 확산할지 논의하는 자리'. 발표 30~40분 + 데모 + Q&A 상정. 슬라이드가 많으니 강조 슬라이드 중심으로.")

# --- 2. Executive Summary ---
current_section_idx = 0
_es = make_content_slide("한 장 요약 (Executive Summary)", [
...
set_notes(_es, "이 한 장이 발표의 핵심이다. 세 가지 효과(비용·자동화·품질)를 먼저 각인시키고, 뒤 슬라이드는 그 근거. "
               "숫자는 '추정'과 '실측'을 구분해 말할 것. 'AI 99% 작성'은 반드시 '사람이 방향·검증'과 함께 언급(핵심 방어 포인트).")

# --- 핵심 효과 요약 '한 장' (기억할 4가지) ---
_impact = make_kpi_slide(
    "핵심 효과 — 이것만 기억하세요",
    [
        ("8~11배", "비용 절감 (3.5~5.5억→4,500만)"),
        ("6~8배", "기간 단축 (2.5~3년→4.5개월, 1인 기준)"),
        ("연 400h+", "반복 운영업무 자동화"),
        ("65건+", "릴리즈 전 결함 선제 발견"),
    ],
    subtitle="1인+AI 개발 사례의 정량 효과 (상세 근거는 이후 슬라이드)")
set_notes(_impact, "임원이 기억할 단 4개 숫자. 비용·기간은 '추정', 자동화 400시간은 '보수적 하한 추정(정성효과 제외)', "
                   "65건+는 실측(파일:라인 근거 있음). 여기서 청중 반응을 보고 관심 큰 항목을 뒤에서 강조.")
...
make_chart_slide(
    "구현 범위 — 영역별 기능 수",
    ["기타·편의", "Digital Twin", "서버 모니터링", "대시보드", "설정·권한",
     "인프라 관리", "알림 관리", "랙 관리", "워크로드", "용량·리포트", "운영 자동화", "Discovery"],
    [23, 22, 21, 17, 11, 10, 8, 6, 5, 4, 4, 4],
    chart_type="bar",
    subtitle="12개 카테고리 · 총 135개 기능 — DCIM 핵심 영역을 폭넓게 커버 (features.md 기준)",
    footnote="※ features.md의 '✅ 완료' 항목 집계.")
...
# --- 라이브 데모 시나리오 ---
_demo = make_content_slide("라이브 데모 시나리오 (실제 화면 시연)", [
    "1. 대시보드 — 전체 인프라 상태 한눈에, 이상 서버(디스크 70%+·온도) 즉시 포착",
    "2. 서버 상세 — CPU/메모리/디스크/온도/전력/PCIe + 파일시스템(NFS 포함) 실시간 확인",
    "3. BMC 원격 전원 제어 — 웹에서 재부팅(2단계 확인) → 감사로그에 '누가·언제·왜' 자동 기록",
    "4. Digital Twin — Room→Rack→장비로 물리 위치 탐색 + 랙 열지도(heatmap)",
    "5. 알림·용량 — 알림 규칙/이력, 12개월 용량 예측 + PDF 리포트 자동 생성",
    "6. (편의) ⌘K 전역 검색, 한↔영 전환, 다크모드",
    "",
    "→ 시연 후 질의응답. 실데이터가 보이는 운영 화면으로 진행.",
], footnote="※ 캡처/시연 순서는 docs/screenshot-checklist.md 참조. 민감정보(IP/호스트명)는 필요 시 모자이크.")
set_notes(_demo, "가능하면 라이브 시연. 불가하면 캡처로 대체(screenshot-checklist.md). "
                 "핵심 임팩트 포인트: ②한 화면에 다 모임, ③웹에서 전원제어+감사추적, ④물리위치 시각화. 5분 내로.")
...
_roi = make_kpi_slide(
    ...
    subtitle="1회성 투자 4,500만 원으로 외주 대비 3억+ 절감 + 매년 반복업무 400시간+ 자동화 → 회수 사실상 즉시")
set_notes(_roi, "의사결정용 슬라이드. 메시지: '이미 만들어졌고, 외주 대비 3억+ 아꼈으며, 앞으로도 매년 자동화 효과가 지속된다.' "
                "추가 투자 요청(v1.0 테스트·리팩터링)의 정당성도 여기서 연결: 소액 재투자로 견고성 확보.")

# --- FP 구성비 도넛 (규모 근거) ---
make_pie_slide(
    "규모 산정 근거 — 기능점수(FP) 구성",
    ["조회(EQ) ~180", "출력(EO) ~175", "내부파일(ILF) ~170", "입력(EI) ~100", "외부연계(EIF) ~15"],
    [180, 175, 170, 100, 15],
    subtitle="구성요소 기반 개략 산정 → 미조정 ~640, 보정 후 약 700 FP. 외주 견적의 근거",
    footnote="※ IFPUG 스타일 개략 추정(공식 count 아님). 백파이어링 교차검증 ~880 FP(과대). 채택 ~700 FP. 상세: cost-comparison.")
...
make_pie_slide(
    ...
    ["테스트 있음 (1)", "테스트 없음 (73)"],
    [1, 73],
    subtitle="API 라우트 74개 중 테스트 1개(약 1.4%) · 화면 렌더링 테스트 0 · E2E 1스펙 — v1.0 최우선 투자 대상",
    footnote="※ 단위 테스트 11파일/72케이스는 존재하나 유틸·쿼리 중심. 위험도 순 보강: ①RBAC ②알림 엔진 ③전원 제어.",
    colors=[ACCENT_GREEN, RGBColor(0xE0, 0x6A, 0x6A)])
...
# --- 장애 대응 등급별 목표 복구 시간 (막대) ---
make_chart_slide(
    "장애 대응 체계 — 등급별 목표 복구 시간",
    ["P0\n전체 중단", "P1\n주요기능·데이터위험", "P2\n기능 저하", "P3\n부분 저하"],
    [15, 60, 240, 480],
    chart_type="column",
    highlight_idx=0,
    value_labels=["15분 이내", "1시간 이내", "4시간 이내", "당일"],
    subtitle="장애 시나리오 20종 + 대화형 복구 도구 이미 구축 · 단위: 분(P3는 당일 기준 예시)",
    footnote="※ operations-plan.md ② 복구 우선순위 매트릭스. P0는 15분 내 대응(강조).")
...
make_flow_slide(
    ...
    ["1단계: 템플릿 패키지화\n(규칙·에이전트·스킬·훅)\n~1개월",
     "2단계: 파일럿 1~2개 부서\n(내부 도구·자동화부터)\n~2~3개월",
     "3단계: 사내 가이드·교육\n확산 (반나절 과정)\n지속"],
    subtitle="한 사람의 사례를 조직의 재사용 자산으로 — 검증된 규칙·도구를 복사해 시작",
    note="※ 적합: 내부 관리도구·대시보드·PoC·반복 문서작업(도메인 전문가 직접 제작). 부적합: 고객대면·고가용성·보안 최상위. "
         "필수 조건: 규칙 체계 + Data-First + 도메인 감독자 + 단일 진실 공급원.")

make_quote_slide("버그가 없는 소프트웨어는 없습니다. 차이는 ‘누가 먼저 찾느냐’입니다. 우리는 AI에게 먼저 찾게 했습니다.",
                 "— DC Express 프로젝트, 그리고 다음 프로젝트를 향해")

# --- 예상 Q&A (발표자 대비) ---
_qa = make_content_slide("예상 질의응답", [
    "Q. AI가 99%를 짰다면, 아무나 할 수 있는 것 아닌가?",
    "  A. 요건 정의·도메인 판단·검증·의사결정은 사람의 몫. 'AI ≠ 자동', 역량 있는 운전자가 전제.",
    "Q. 품질·유지보수는 괜찮은가? (테스트가 부족하다던데)",
    "  A. 한계 인정 — API 테스트 1.4%. 단, AI 병렬 감사로 릴리즈 전 65건+ 선제 발견. v1.0에서 테스트 최우선 투자.",
    "Q. 1인 개발이면 그 사람이 없으면 어떻게 되나? (버스팩터)",
    "  A. 인수인계 문서 체계 완비 + 백업 담당자 지정·교육 계획(운영 계획안 반영).",
    "Q. 보안은 신뢰할 수 있나?",
    "  A. RBAC·조직격리·감사로그 기본 탑재. 발견된 권한 우회 즉시 수정 + 회귀 테스트로 재발 방지.",
    "Q. 다른 부서에도 적용 가능한가?",
    "  A. 내부 도구·대시보드에 적합. 규칙·도구 템플릿화로 파일럿부터 단계적 확산 가능.",
])
set_notes(_qa, "가장 나올 법한 5개. 방어 톤이 아니라 '이미 대비했다'는 톤으로. "
               "테스트/버스팩터 질문은 오히려 v1.0 투자·백업담당자 지정의 근거로 전환. 보안은 '즉시 수정+테스트'로 신뢰 회복 스토리.")
